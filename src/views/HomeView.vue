<template>
  <main class="container dark:text-white text-slate-600 md:mt-16 mt-24">
    <div class="pt-4 mb-8 relative">
      <div class="relative">
        <label for="default-search" class="mb-2 text-sm font-medium text-gray-900 sr-only dark:text-white">Search</label>
          ...
          <input type="search" id="default-search" v-model="searchQuery" @input="getSearchResults" placeholder="Masukan Nama Kota " class="block w-full p-4 ps-10 text-sm text-gray-900 border border-gray-300 rounded-lg bg-gray-50 focus:ring-slate-500 focus:border-slate-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-slate-500 dark:focus:border-slate-500" />
      </div>
      <ul class="z-20 mt-4 rounded-xl border-2 shadow-slate-500 border-slate-400 absolute bg-slate-200 text-slate-800 dark:bg-gray-900 dark:text-slate-200 w-full shadow-md py-2 px-1 top-[66px]" v-if="mapboxSearchResults">
